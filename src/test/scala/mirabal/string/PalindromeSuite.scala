...
class PalindromeSuite extends FunSuite with Matchers {
  ...
  test("find palindromes capping results to max") {
    val result = "sqrrqabccbatudefggfedvwhijkllkjihxymnnmzpop".findPalindromes(3)
    result should contain only(
      PalindromeResult("hijkllkjih", 23, 10),
      PalindromeResult("defggfed", 13, 8),
      PalindromeResult("abccba", 5, 6),
  ...
  test("find palindromes without filtering when max is greater than number of results") {
    val result = "madamqaba".findPalindromes(5)
    result should contain only(
      PalindromeResult("madam", 0, 5),
      PalindromeResult("aba", 6, 3)
    )
